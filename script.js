...
const API_KEY = "d76aa42e82f64d97098277ee6a9b56a0";
...
const getUserCoordinates = () => {
  navigator.geolocation.getCurrentPosition(
    (position) => {
      ...
      const API_URL = `http://api.openweathermap.org/geo/1.0/reverse?lat=${latitude}&lon=${longitude}&appid=${API_KEY}`;
      fetch(API_URL)
        .then((response) => response.json())
        .then((data) => {
          const { name } = data[0];
        ...
        .catch(() => {
          alert("An error occuredwhile fetching the city name!");
        });
    },
    (error) => {
      if (error.code === error.PERMISION_DENIED) {
        alert(
          "Geolocation request denied.Please reset location permissionto grant access again."
        );
      } else {
        alert("Geolocation requesterror.Please reset location permission.");
      }
    }
  );
};
